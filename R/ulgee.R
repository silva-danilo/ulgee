...
diag_quant <- function(fit.model, X, nsim, show.step=T, random=F, n, label.id, 
  ...
  if(random==T){
    ...
    plot(fit.model$mu.hat[pos], fit.model$rq[pos], xlab="Valor ajustado", 
         ylab="Resíduo quantílico", pch=16, lwd=2, cex.lab=1.5, cex.axis=1.2)
    ...
    qqnorm(fit.model$rq[pos], xlab="Quantis teóricos", ylab="Quantis empíricos",
  ...
  # non random selection
    ...
    # plot quantile residual vs value adjusted
    par(mar=c(5.5,5.5,2,2), mfrow=c(1,2))
    plot(fit.model$mu.hat, fit.model$rq, xlab="Valor ajustado", 
         ylab="Resíduo quantílico", pch=16, lwd=2, cex.lab=1.5, cex.axis=1.2)
    ...
    if(nsim > 0){
      ...
      # plot qq-norm quantile residual 
      faixa <- range(e1, e2) 
      qqnorm(fit.model$rq, xlab="Quantis teóricos", ylab="Quantis empíricos",
             pch=16, lwd=2, cex.lab=1.5, cex.axis=1.2, ylim=faixa, main="")
...
sens_conf <- function(fit.model, c_c, n_c, c_r, n_r, label.id, label.time){
  # plot Bi_c vs index
  par(mar=c(5.5,5.5,2,2), mfrow=c(1,2))
  plot(fit.model$Bi_c, ylab=expression("B"["ij"]), xlab="Índice", cex.lab=1.5,
       cex.axis=1.2, pch=16)
  ...
  # plot Bi_r vs index
  plot(fit.model$Bi_r, ylab=expression("B"["ij"]), xlab="Índice", cex.lab=1.5,
       cex.axis=1.2, pch=16)
